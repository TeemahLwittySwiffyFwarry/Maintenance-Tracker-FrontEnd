...
function ViewAllMachines() {
  ...
  const getFilteredRepairs = (machine) =>
    selectedMonth
      ? machine.repairs.filter((repair) => new Date(repair.time_added).toISOString().slice(0, 7) === selectedMonth)
      : machine.repairs;
  ...
  const getFilteredMachines = () => {
    return machines
      .map((machine) => {
        const filteredRepairs = (machine.repairs || []).filter((repair) =>
          repair.repair_type_name?.toLowerCase().includes(searchTerm.toLowerCase())
        );
  
        // Keep machine if its name matches or it has relevant filtered repairs
        if (
          ...
          return { 
            ...machine, 
            repairs: filteredRepairs // Update repairs to only show filtered ones
          };
        }
        return null;
      })
